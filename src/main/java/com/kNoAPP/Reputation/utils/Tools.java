package com.kNoAPP.Reputation.utils;
...
public class Tools {
    ...
    public static boolean isWithinBlocksFromTwoPoints(Location t, Location loc1, Location loc2) {
    ...
    public static void projTrail(Projectile proj, Particle[] particles) {
		new BukkitRunnable() {
			public void run() {
				if(proj != null && proj.isValid() && !proj.isDead()) for(Particle particle : particles) proj.getWorld().spawnParticle(particle, proj.getLocation(), 1, 0F, 0F, 0F, 0.01);
				else this.cancel();
			}
		}.runTaskTimer(Reputation.getPlugin(), 0L, 3L);
	}
